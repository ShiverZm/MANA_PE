#ifndef SRC_PE_ATTRIBUTES_INCLUDE_MANAPE_CORE_H_
#define SRC_PE_ATTRIBUTES_INCLUDE_MANAPE_CORE_H_
...
// Helper code - not intended to be directly called by
// the library users. May be changed at any time
namespace internal {
// Unicode constants
...
utf_error validate_next(octet_iterator& it, octet_iterator end,
                        uint32_t& code_point) {
    // Save the original value of it so
    // we can go back in case of failure
...
#endif  // SRC_PE_ATTRIBUTES_INCLUDE_MANAPE_CORE_H_
