...
flag_dict IMAGE_OPTIONAL_HEADER_MAGIC = {{"PE32", 0x10b}, {"PE32+", 0x20b}};

flag_dict MACHINE_TYPES = {{"IMAGE_FILE_MACHINE_UNKNOWN", 0},
...
const flag_dict DEBUG_TYPES = {
    {"IMAGE_DEBUG_TYPE_UNKNOWN", 0},       {"IMAGE_DEBUG_TYPE_COFF", 1},
    {"IMAGE_DEBUG_TYPE_CODEVIEW", 2},      {"IMAGE_DEBUG_TYPE_FPO", 3},
    {"IMAGE_DEBUG_TYPE_MISC", 4},          {"IMAGE_DEBUG_TYPE_EXCEPTION", 5},
    {"IMAGE_DEBUG_TYPE_FIXUP", 6},         {"IMAGE_DEBUG_TYPE_OMAP_TO_SRC", 7},
    {"IMAGE_DEBUG_TYPE_OMAP_FROM_SRC", 8}, {"IMAGE_DEBUG_TYPE_BORLAND", 9},
    {"IMAGE_DEBUG_TYPE_RESERVED", 10},     {"IMAGE_DEBUG_TYPE_CLSID", 11},
    {"IMAGE_DEBUG_TYPE_VC_FEATURE", 12},   {"IMAGE_DEBUG_TYPE_POGO", 13},
    {"IMAGE_DEBUG_TYPE_ILTCG", 14},        {"IMAGE_DEBUG_TYPE_MPX", 15}};

const flag_dict GLOBAL_FLAGS = {
    {"FLG_STOP_ON_EXCEPTION", 0x1},
...
const flag_dict WIN_CERTIFICATE_REVISIONS = {{"WIN_CERT_REVISION_1_0", 0x100},
                                             {"WIN_CERT_REVISION_2_0", 0x200}};

const flag_dict WIN_CERTIFICATE_TYPES = {{"WIN_CERT_TYPE_X509", 1},
...
const flag_dict CODEPAGES = {{"IBM EBCDIC US-Canada", 37},
                             {"IBM PC US", 437},
                             {"Thai", 874},
                             {"Japanese", 932},
...
const flag_dict FIXEDFILEINFO_FILEOS = {{"VOS_UNKNOWN", 0x00000000},
                                        {"VOS_DOS", 0x00010000},
...
const flag_dict FIXEDFILEINFO_FILETYPE = {
    {"VFT_UNKNOWN", 0x00000000},   {"VFT_APP", 0x00000001},
    {"VFT_DLL", 0x00000002},       {"VFT_DRV", 0x00000003},
...
std::string translate_to_flag(int value, const flag_dict &dict) {
    auto it = std::find_if(dict.begin(), dict.end(), [value](const auto &pair) {
        return value == pair.second;
    });
    return (it != dict.end()) ? it->first : "UNKNOWN";
}

std::vector<std::string> translate_to_flags(int value, const flag_dict &dict) {
    std::vector<std::string> res;
    for (auto it = dict.begin(); it != dict.end(); ++it) {
        // The flag is present in the value
