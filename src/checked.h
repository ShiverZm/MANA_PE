#ifndef SRC_PE_ATTRIBUTES_INCLUDE_MANAPE_CHECKED_H_
#define SRC_PE_ATTRIBUTES_INCLUDE_MANAPE_CHECKED_H_
...
uint32_t previous(octet_iterator& it, octet_iterator pass_start) {
    octet_iterator end = it;
    while (utf8::internal::is_trail(*(--it)))
        if (it == pass_start) throw invalid_utf8(*it);
    ...
    bool operator==(const iterator& rhs) const {
        if (range_start != rhs.range_start || range_end != rhs.range_end)
            throw std::logic_error(
                "Comparing utf-8"
                "iterators defined with different ranges");
        return (it == rhs.it);
    }
...
#endif  // SRC_PE_ATTRIBUTES_INCLUDE_MANAPE_CHECKED_H_
